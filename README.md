## 系统配置 (`config.yaml`/`config.json`) 参数说明文档

本文档旨在详细解释配置文件中的各个参数（推荐使用 `config.yaml`，若不存在则使用 `config.json`）的含义、作用以及建议的配置方式。
...
1.  **启动与初始化 (`main.py`)**
    *   程序启动时，主进程优先读取 `config.yaml`；若不存在则回退到 `config.json`（兼容解析 JSON）。
