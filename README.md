...
本仓库只保留支撑产线程序运行和打包所需的文件：主程序运行链路、算法模块、相机 SDK 包装、剔废/报警/上报模块、依赖声明、配置模板、已有 ROI 模板和打包静态资源。产线机器上已经存在的现场资产，例如各机台实际使用的 ROI 模板、相机 MAC/IP 绑定、串口号、服务器地址和回放数据，以现场机器为准；部署或替换代码前，需要让 `config*.yaml` 中的路径和现场文件保持一致。

不纳入仓库的内容包括标注工具、仿真脚本、离线评估脚本、测试脚本、设计草稿、运行日志、检测输出、回放压缩包、Nuitka 打包目录、历史备份文件、profile 文件和一次性统计报表。这些文件不直接支撑产线主程序启动和运行，适合保留在产线机器、本地工作区或单独归档。

## License

本项目使用 PolyForm Noncommercial License 1.0.0，仅允许非商业用途。
...
- 启动与调度：`main.py`
- 相机采集：`camera_process.py`、`camera_manager.py`、`MVGigE.py`、`GigECamera_Types.py`、`MVGigE.dll`
- 图像算法：`processing_worker.py`、`fused_image_processor.py`、`image_processor_hough.py`、`image_processor_hough_dark.py`、`generate_test_image.py`
- API 与状态：`api_server.py`、`state_machine.py`、`utils.py`
- 通信与统计：`http_client.py`、`server_comms.py`、`yield_manager.py`
- 剔废与报警：`rejection_controller.py`、`rejection_control.py`、`alarm_light_controller.py`、`remote_alarm_server.py`、`remote_alarm_server.json`
- 配置和模板：`config.yaml`、`config1.yaml`、`config2.yaml`、`config3.yaml`、`roi_averaged_by_group_CORRECTED.json`、`cam1_roi_averaged_by_group.json`、`cam5_roi_averaged_by_group.json`、`line3cam3_roi_averaged_by_group.json`
- 打包和依赖：`pyproject.toml`、`uv.lock`、`.python-version`、`icon.ico`
...
- 主程序运行闭包、配置模板、SDK 包装、依赖元数据和文档进入 Git。
- 运行输出、回放数据、日志、报表、备份和打包产物不进入 Git。
- 标注、仿真、离线评估、测试和 profile 工具不进入交付仓库。
