...
def find_and_analyze_defects(edges, roi_gray, roi_dims, params, pixels_per_mm: float):
    ...
    shield_radius = _get_dist_px(p_defect, "CHIPPING_ENDPOINT_SHIELD_RADIUS_MM", "CHIPPING_ENDPOINT_SHIELD_RADIUS", 0.0, pixels_per_mm)
...
def process_frame(image_gray, template_rois, config, thread_workers: int | None = None):
    """与原 process_image_from_memory_parallel 类似, 但:
    - 返回 (report, final_image, per_roi_images)
    - per_roi_images: list[(roi_idx, roi_report, roi_color)] 用于 data_collector 保存单独 ROI 图。
    - 可由外部传入 thread_workers 覆盖 system_params.roi_threads.
    """
    ...
    def _safe_roi_hough(i, r):
        try:
            # 兼容 r 中可能使用 w/h 或 width/height 字段
            if 'width' in r and 'w' not in r:
                r = {**r, 'w': r.get('width')}
            if 'height' in r and 'h' not in r:
                r = {**r, 'h': r.get('height')}
            ...
            w, h = int(r.get('w', r.get('width',0))), int(r.get('h', r.get('height',0)))
